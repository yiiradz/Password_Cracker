...
// Java program to calculate MD5 hash value
    ...
    public static void main(String args[]) throws NoSuchAlgorithmException, IOException {
        ...
        /*
        String [] appendedWords = new String[12700];
        String [] hashedAppended = new String[12700];
        String [] leetWords = new String [12700];
        String [] hashedLeet = new String [12700];
        int pos = 0;
        while(scanner.hasNext())
        {
            String line = scanner.nextLine();
            String line2=line;
            Dictionary_Generator.appendSymbolsToWords(line);
            System.out.println(line);
            appendedWords[pos]=line;
            System.out.println(appendedWords[pos]);
            hashedAppended[pos]=getMd5(appendedWords[pos]);
            System.out.println(hashedAppended[pos]);
            Dictionary_Generator.l33tSp34kTranslation(line2);
            leetWords[pos]=line2;
            hashedLeet[pos]=getMd5(leetWords[pos]);
            for(int i=0; i<hashes.length; i++)
            {
                if(hashes[i].equals(hashedAppended[pos]))
                {
                    System.out.println(usernames[i]+":"+appendedWords[pos]);
                    break;
                }
                if(hashes[i].equals(hashedLeet[pos]))
                {
                    System.out.println(usernames[i]+":"+leetWords[pos]);
                    break;
                }
            }
            pos++;
...
/*
        String [] myNewWords = new String [12700];
        String [] myEditedWords = new String [12700];
        String [] myHWords = new String [12700];
        //for pws in pos 15, 16, 17, 18, and 19
        String [] salts = {"erty", "zxcv", "hjkl", "uiop", "asdf"};
        //making the generator here
        while(scanner.hasNextLine()||scanner.hasNextInt()) {
            //each word is here
            myNewWords[i] = scanner.next();
            //plain word to hash
            myHWords[i]=getMd5(myNewWords[i]);
            //compare hashed words to hashes of pws and print
            for(int place=0; place < 20; place++) {
                if (myHWords[i].equals(hashes[place])) {
                    System.out.println(usernames[place] + ":" + myNewWords[i]);
                    //break;
                }
            }

            for(int j=0; j<myNewWords.length; j++)
            {
                myEditedWords[j]=myNewWords[j]+salts[0];
                myHWords[j]=getMd5(myEditedWords[j]);
                if(myHWords.equals(usernames[15]))
                {
                    System.out.println(usernames[15]+":"+myHWords[j]);
                }
                myEditedWords[j]=myNewWords[j]+salts[1];
                myHWords[j]=getMd5(myEditedWords[j]);
                if(myHWords.equals(usernames[16]))
                {
                    System.out.println(usernames[16]+":"+myHWords[j]);
                }
                myEditedWords[j]=myNewWords[j]+salts[2];
                myHWords[j]=getMd5(myEditedWords[j]);
                if(myHWords.equals(usernames[17]))
                {
                    System.out.println(usernames[17]+":"+myHWords[j]);
                }
                myEditedWords[j]=myNewWords[j]+salts[3];
                myHWords[j]=getMd5(myEditedWords[j]);
                if(myHWords.equals(usernames[18]))
                {
                    System.out.println(usernames[18]+":"+myHWords[j]);
                }
                myEditedWords[j]=myNewWords[j]+salts[4];
                myHWords[j]=getMd5(myEditedWords[j]);
                if(myHWords.equals(usernames[19]))
                {
                    System.out.println(usernames[19]+":"+myHWords[j]);
                }
            }
            //all words are plaintext in myWords
            i++;
            //now I want to add the generator to do all the stuff to the words and test periodically

            //compare hashed things each time around to the new words
        }
*/
